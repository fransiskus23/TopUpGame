...
class PaymentController extends Controller
{
    ...
    public function depositConfirm(Request $request)
    {
        ...
        $order = Fund::where('transaction', $track)->orderBy('id', 'DESC')->with(['gateway','user'])->firstOrFail();

        if ($order->status != 0) {
            session()->flash('error', 'Invalid Payment Request');
            if(optional($order->fundable)->category_id){
                return redirect()->route('topUp.details',[slug((optional($order->fundable)->category->details->name)??'game-top-up'),optional($order->fundable)->category_id]);
            }

            if(optional($order->fundable)->voucher_id){
                return redirect()->route('voucher.details',[slug((optional($order->fundable)->voucher->details->name)??'voucher'),optional($order->fundable)->voucher_id]);
            }

            if(optional($order->fundable)->gift_card_id){
                return redirect()->route('giftCard.details',[slug((optional($order->fundable)->giftCard->details->name)??'gift-card'),optional($order->fundable)->gift_card_id]);
            }
            if(optional($order->fundable)->sell_post_id){
                return redirect()->route('sellPost.details',[slug(optional($order->fundable)->sellPost->title)??'sell-post',optional($order->fundable)->sell_post_id]);
            }
    ...
    public function fromSubmit(Request  $request)
    {
        ...
        if(optional($data->fundable)->sell_post_id){
            $sellPost = optional($data->fundable)->sellPost;
            $sellPost->payment_status = 3;
            $sellPost->save();

            $fundable = $data->fundable;
            $fundable->payment_status = 3;
            $fundable->save();
        }
